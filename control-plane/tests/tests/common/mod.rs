...
impl Cluster {
    ...
    async fn new(
        ...
        components
            .wait_on(&cluster.composer, std::time::Duration::from_secs(10))
            .await?;
...
impl ClusterBuilder {
    ...
    pub fn with_pools(mut self, count: u32) -> Self {
        let existing_pools = self.pools.len();
        for _ in 0 .. count {
        ...
        for pool_index in 0 .. self.pools.len() {
            ...
            for i in 0 .. count {
                pool.replicas.push(v0::CreateReplica {
                    ...
                    uuid: Cluster::replica(pool_index, i),
                    pool: pool.id(),
                    size,
                    thin: false,
                    share: share.clone(),
