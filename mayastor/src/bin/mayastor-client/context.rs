...
use bytes::Bytes;
use clap::ArgMatches;
use http::uri::{Scheme, Uri, Authority, PathAndQuery};
...
impl Context {
    pub(crate) async fn new(matches: &ArgMatches<'_>) -> Result<Self, Error> {
        ...
        // TODO: This can be significantly cleaned up when we update tonic 0.1 and its deps.
        let host = if let Some(host) = matches.value_of("bind") {
            let uri = Uri::from_shared(Bytes::from(host)).context(InvalidUriBytes)?;
            let mut parts = uri.into_parts();
            if parts.scheme.is_none() {
                parts.scheme =
                    Scheme::from_str("http").ok();
            }
            if let Some(ref mut authority) = parts.authority {
                if authority.port_part().is_none() {
                    parts.authority = Authority::from_shared(
                        Bytes::from(format!("{}:{}", authority.host(), 10124))
                    ).ok()
                }
            }
